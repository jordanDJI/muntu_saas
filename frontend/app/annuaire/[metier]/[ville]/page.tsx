...
export default async function AnnuaireVillePage({
  ...
  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: `${m.labelPlural} ${v.labelPrep}`,
    ...
    <main className="min-h-screen bg-white">
        ...
        {listings.length === 0 ? (
          <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-8 text-center">
            <p className="text-indigo-800 font-semibold mb-2">
              Vous êtes {m.labelArticle} {v.labelPrep} ?
            </p>
            ...
            <Link
              href="/"
              className="bg-indigo-700 text-white font-bold px-6 py-2.5 rounded-lg hover:bg-indigo-800 transition-colors text-sm"
            >
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {listings.map((l: any) => (
              <Link
                ...
                className="border border-gray-200 rounded-xl p-5 hover:shadow-md hover:border-indigo-300 transition-all group"
              >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-indigo-600 font-medium ml-auto group-hover:underline">
                    Voir le profil →
                  </span>
                </div>
              </Link>
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            mainEntity: [
              {
                "@type": "Question",
                name: `Comment trouver ${m.labelArticle} ${v.labelPrep} ?`,
                acceptedAnswer: {
                  "@type": "Answer",
                  text: `L'annuaire Klientys liste les ${m.labelPlural.toLowerCase()} ${v.labelPrep} avec prise de RDV directe. Chaque professionnel a vérifié ses zones d'intervention.`,
                },
              },
              {
                "@type": "Question",
                name: `Comment s'inscrire en tant que ${m.labelArticle} ${v.labelPrep} ?`,
                acceptedAnswer: {
                  "@type": "Answer",
      ...
      <section className="bg-gray-50 border-t py-12 px-6 text-center">
        <p className="text-gray-700 font-semibold mb-4">
          Vous êtes {m.labelArticle} {v.labelPrep} et vous n'êtes pas listé ?
        </p>
        <Link
          href="/"
          className="bg-indigo-700 text-white font-bold px-6 py-2.5 rounded-lg hover:bg-indigo-800 transition-colors text-sm"

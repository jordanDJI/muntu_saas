...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Navbar fixe */}
      <nav className="fixed top-0 inset-x-0 z-50 h-14 bg-white border-b border-gray-200">
        <div className="max-w-6xl mx-auto h-full px-4 grid grid-cols-3 items-center">

          {/* Colonne gauche — logo */}
          <div className="flex items-center">
            <Link href="/" className="font-bold text-indigo-600 text-sm hover:text-indigo-700 transition-colors">
              Présence&nbsp;Pro
            </Link>
          </div>
          ...
          <div className="flex items-center justify-end gap-1">
            {tenantSlug && !pathname.startsWith("/dashboard/site-builder") && (
              <a
                ...
                className="hidden md:inline-flex items-center gap-1.5 text-sm text-indigo-600 hover:text-indigo-700 px-2.5 py-1.5 rounded-lg hover:bg-indigo-50 transition-colors"
              >
            ...
            <Link
              ...
              className={`hidden md:inline-flex items-center justify-center w-8 h-8 rounded-lg transition-colors ${pathname.startsWith("/dashboard/settings") ? "bg-indigo-50 text-indigo-700" : "text-gray-500 hover:text-gray-800 hover:bg-gray-100"}`}
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
            ...
            <button
              onClick={logout}
              className="hidden md:inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-800 px-3 py-1.5 rounded-lg hover:bg-gray-100 transition-colors"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
      ...
      {/* Menu mobile */}
      {menuOpen && (
        <div className="fixed top-14 inset-x-0 z-40 bg-white border-b border-gray-200 p-3 space-y-1 md:hidden shadow-md">
          {NAV.map((n) => {
            const active = pathname === n.href || (n.href !== "/dashboard" && pathname.startsWith(n.href));
            return (
              <Link
          ...
          <Link
            href="/dashboard/settings"
            onClick={() => setMenuOpen(false)}
            className={`flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-medium ${pathname.startsWith("/dashboard/settings") ? "bg-indigo-50 text-indigo-700" : "text-gray-700 hover:bg-gray-100"}`}
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/>
              <circle cx="12" cy="12" r="3"/>
